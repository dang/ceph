...
def main_fix(args):
    ...
    # Relabel/chown all files under /var/lib/ceph/ recursively and in parallel
    for directory in glob.glob('/var/lib/ceph/*'):
        fix_table[directory] = ('ceph', 'ceph', False, True)
    ...
    if args.all:
        for directory in fix_table:
            uid, gid, blocking, recursive = fix_table[directory]
            ...
            if blocking:
                ...
                bg_processes.append(command_init(c))
    ...
    if args.selinux:
        for directory in fix_table:
            ...
            if blocking:
                ...
                if ret:
                    LOG.error("Failed to restore SELinux labels for " + directory)
                    LOG.error(err)
    ...
    for process in bg_processes:
        ...
        if ret:
            LOG.error("A background process failed")
            LOG.error(err)
