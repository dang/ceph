                         ...
                         bool force)
{
  ...
  if (store->get_local_zone()->get_id() == master_zone) {
...
{
  const RGWRealm& realm = store->get_local_zone()->get_realm();
  const RGWZoneGroup& zonegroup = store->get_local_zone()->get_zonegroup();
...
{
  rgw_zone_id zone_id = opt_target_zone.value_or(store->get_local_zone()->get_id());
...
int main(int argc, const char **argv)
{
  ...
  if (!placement_id.empty() ||
    ...
    if (!store->get_local_zone()->get_params().valid_placement(target_rule)) {
...
#define MAX_LC_LIST_ENTRIES 100
  ...
  if (opt_cmd == OPT::DATA_SYNC_RUN) {
    ...
    int ret = static_cast<rgw::sal::RadosStore*>(store)->svc()->sync_modules->get_manager()->create_instance(g_ceph_context, static_cast<rgw::sal::RadosStore*>(store)->svc()->zone->get_zone().tier_type,
        store->get_local_zone()->get_params().tier_config, &sync_module);
    if (ret < 0) {
