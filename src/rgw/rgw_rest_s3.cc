...
#if 1 /* DANG_RADOSSTORE */
#include "rgw_rest_s3_rados.h"
#endif /* DANG_RADOSSTORE */
...
void RGWGetBucketLocation_ObjStore_S3::send_response()
{
  ...
  int ret = store->get_local_zone()->get_zonegroup(s->bucket->get_info().zonegroup, zonegroup);
  if (ret >= 0) {
...
#if 1 /* DANG_RADOSSTORE */
    RGWBucketSyncPolicyHandlerRef sync_policy_handler{nullptr};
    int r = store->get_sync_policy_handler(nullptr, std::nullopt, std::nullopt, &sync_policy_handler, null_yield);
    if (r < 0 || !sync_policy_handler ||
...
int RGWHandler_REST_S3::postauth_init(optional_yield y)
{
  ...
  if (mfa) {
    ret = s->user->verify_mfa(s, y, string(mfa), &s->mfa_verified);
  }
